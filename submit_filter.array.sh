...
find "$IN" -maxdepth 1 -type f \( -name '*.R1*.fastq' -o -name '*.R1*.fastq.gz' \) -exec sh -c 'for f; do echo "$f"; done >> "$TMPDIR/r1_files.txt"' sh {} +
find "$IN" -maxdepth 1 -type f \( -name '*.R2*.fastq' -o -name '*.R2*.fastq.gz' \) -exec sh -c 'for f; do echo "$f"; done >> "$TMPDIR/r2_files.txt"' sh {} +
...
cat "$TMPDIR/r1_files.txt" > "$TMPDIR/all_files.txt"
num_jobs=$(wc -l < "$TMPDIR/all_files.txt")
echo $num_jobs

sbatch --array=1-$num_jobs%10 filter.array.sbatch
